...
class MMTFDecoder():
    ...
    def decode_data(self, input_data):
        ...
        self.cartnX = array_converters.convert_ints_to_floats(
            array_decoders.delta_decode(
                array_converters.combine_integers(
                    array_converters.convert_bytes_to_ints(input_data["xCoordSmall"], 2),
                    array_converters.convert_bytes_to_ints(input_data["xCoordBig"], 4))),
            COORD_DIVIDER )
        self.cartnY = array_converters.convert_ints_to_floats(
            array_decoders.delta_decode(
                array_converters.combine_integers(
                    array_converters.convert_bytes_to_ints(
                        input_data["yCoordSmall"],2),
                    array_converters.convert_bytes_to_ints(input_data["yCoordBig"], 4))),
            COORD_DIVIDER )
        self.cartnZ = array_converters.convert_ints_to_floats(
            array_decoders.delta_decode(
                array_converters.combine_integers(
                    array_converters.convert_bytes_to_ints(
                        input_data["zCoordSmall"],2),
                    array_converters.convert_bytes_to_ints(input_data["zCoordBig"], 4))),
            COORD_DIVIDER)
        # Run length decode the occupancy array
        if "bFactorSmall" in input_data and "bFactorBig" in input_data:
            self.b_factor = array_converters.convert_ints_to_floats(
                array_decoders.delta_decode(
                    array_converters.combine_integers(
                        array_converters.convert_bytes_to_ints(input_data["bFactorSmall"], 2),
                        array_converters.convert_bytes_to_ints(input_data["bFactorBig"], 4))),
                OCC_B_FACTOR_DIVIDER)
        else:
            self.b_factor = []
        if "occupancyList" in input_data:
            self.occupancy = array_converters.convert_ints_to_floats(
                array_decoders.run_length_decode(
                    array_converters.convert_bytes_to_ints(
        ...
        if "atomIdList" in input_data:
            self.atom_id = array_decoders.delta_decode(
                array_decoders.run_length_decode(
                    array_converters.convert_bytes_to_ints(input_data["atomIdList"], 4)))
        else:
            self.atom_id = []
        # Run length encoded
        if "altLocList" in input_data:
            self.alt_id = array_converters.convert_ints_to_chars(
                array_decoders.run_length_decode(
                    array_converters.convert_bytes_to_ints(input_data["altLocList"], 4)))
        else:
            self.alt_id = []
        if "insCodeList" in input_data:
            self.insertion_code_list = array_converters.convert_ints_to_chars(
                array_decoders.run_length_decode(
                    array_converters.convert_bytes_to_ints(input_data["insCodeList"], 4)))
        else:
            self.insertion_code_list = []
        # Get the group_number
        self.group_num = array_decoders.delta_decode(
            array_decoders.run_length_decode(
                array_converters.convert_bytes_to_ints(input_data["groupIdList"], 4)))
        ...
        if "sequenceIndexList" in input_data:
            self.seq_res_group_list = array_decoders.delta_decode(
                array_decoders.run_length_decode(
                    array_converters.convert_bytes_to_ints(input_data["sequenceIndexList"], 4)))
        else:
